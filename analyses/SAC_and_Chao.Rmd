...
total_honey_bees <- dat1 %>% 
  mutate(Site = factor("All sites")) %>% 
  filter(Short.label.name == "Apis mellifera") %>% 
  select(Site, Short.label.name) %>% 
  group_by(Site) %>% tally(name = "A_mellifera") %>% 
  mutate(total = nrow(dat1))
  

left_join(honey_bees, total_bees, by = "Site") %>% 
  bind_rows(total_honey_bees, .) %>% 
  mutate(Percent = 100 * (A_mellifera/total)) %>% 
  kable(., format = "markdown", digits = 2)

honey_bees_total <- left_join(honey_bees, total_bees, by = "Site") %>% 
  bind_rows(total_honey_bees, .) %>% 
  mutate(Percent = 100 * (A_mellifera/total)) 

write.csv(honey_bees_total,
          "writing/supplemental/honey_bees.csv", row.names = F)
...
singles <- dat2 %>% 
  filter(Genus != "Hylaeus") %>% 
  group_by(Short.label.name) %>% 
  tally() %>% 
  filter(n == 1)

singletons <- semi_join(dat2, singles, by = "Short.label.name") %>% 
  select(Short.label.name, Site) %>% 
  rename(Species = Short.label.name) %>% 
  arrange(Site)

nrow(singletons)

kable(singletons, format = "markdown")

write.csv(singletons,
          "writing/supplemental/singletons.csv", row.names = F)

```

# Top 10 most abundant species
```{r most_abundant, echo = F}
...
write.csv(most_abundant,
          "writing/supplemental/most_abundant_10.csv", row.names = F)

```

# Species collected in all 3 sites
```{r all_three_sites_species, echo = F}

species_all3_sites <- dat2 %>% 
  group_by(Short.label.name) %>% 
  summarize(Site_count = n_distinct(Site)) %>%
  filter(Site_count == 3) %>% 
  semi_join(x = dat2, y = ., by = "Short.label.name") %>% 
...
nrow(species_all3_sites)

kable(species_all3_sites, format = "markdown")

write.csv(species_all3_sites,
          "writing/supplemental/species_found_all_3_sites.csv",
          row.names = F)

```

# Only collected by net
```{r species_only_net_collected, echo = F}

net_only <- dat2 %>% 
  group_by(Collection.Method, Short.label.name) %>% 
  tally() %>% 
  pivot_wider(names_from = Collection.Method,
              values_from = n,
              values_fill = list(n = 0)) %>% 
  filter(N >= 0 & `T` == 0) %>% 
  rename(Species = Short.label.name,
...
kable(net_only, format = "markdown")

write.csv(net_only,
          "writing/supplemental/net_collected_only.csv", row.names = F)

```

# Species for which ZERO males were collected (net and trap)
```{r only_females_species, echo = F}

no_males <- dat2 %>% 
  group_by(Short.label.name, Sex) %>% 
  tally() %>% 
...
kable(no_males, format = "markdown")

write.csv(no_males,
          "writing/supplemental/no_males.csv", row.names = F)

```

# All species with counts for site, collection method, and sex
```{r all_species_table, echo = F}
...
wide_species <- left_join(all_species,
                          site_species,
                          by = "Short.label.name")
wide_species <- left_join(wide_species, collection_species,
                          by = "Short.label.name")
wide_species <- left_join(wide_species, sex_species,
                          by = "Short.label.name")

wide_species <- wide_species %>% 
  rename(Species = Short.label.name,
         Trap = `T`,
         Net = N) %>% 
  arrange(desc(Total))

kable(wide_species, format = "markdown")

write.csv(wide_species,
          file = "writing/supplemental/wide_species.csv")
```
...
chao_site_year <- dat2 %>% 
  filter(Collection.Method == "T") %>% 
  group_by(SiteYear, Short.label.name) %>% 
  tally(name = "count") %>% 
...
chao_site_year <- data.frame(
  chao1 = sapply(seq_along(chao_site_year), function(i){
    chao1(chao_site_year[[i]])}),
  SiteYear = colnames(chao_site_year))

# Trap by year
chao_trap_year <- dat2 %>% 
  filter(Collection.Method == "T") %>% 
  group_by(Year, Short.label.name) %>% 
  tally(name = "count") %>% 
...
chao_trap_year <- data.frame(
  chao1 = sapply(seq_along(chao_trap_year), function(i){
    chao1(chao_trap_year[[i]])}),
  Year = colnames(chao_trap_year))

# Trap and Net by year
chao_trapnet_year <- dat2 %>% 
  group_by(Year, Short.label.name) %>% 
  tally(name = "count") %>% 
  pivot_wider(names_from = Year,
              values_from = count,
...
chao_trapnet_year <- data.frame(
  chao1 = sapply(seq_along(chao_trapnet_year), function(i){
    chao1(chao_trapnet_year[[i]])}),
  Year = colnames(chao_trapnet_year))
```
From Anne Chao 1989:
...
chao_trap_year <- chao_trap_year %>%
  mutate(Site = "All Sites")

chao_trapnet_year <- chao_trapnet_year %>% 
  mutate(Site = "All Sites (N & T)")

chao_site_year <- chao_site_year %>% 
  separate_wider_delim(SiteYear,
                       delim = "_",
                       names = c("Site", "Year"))

chao_fig <- bind_rows(chao_trapnet_year,
                      chao_trap_year,
                      chao_site_year) %>% 
  rename(rich = chao1)
...
site_year <- dat2 %>% 
  filter(Collection.Method == "T") %>% 
  group_by(Short.label.name, SiteYear) %>% 
  tally() %>% 
  group_by(SiteYear) %>% 
  tally() %>% 
  separate_wider_delim(SiteYear,
                       ...
                       names = c("Site", "Year")) %>% 
  mutate(Site2 = Site,
         Year = factor(Year)) %>% select(-Site) %>% 
  rename(Site = Site2)

trap_year <- dat2 %>% 
  filter(Collection.Method == "T") %>% 
  group_by(Short.label.name, Year) %>% 
  tally() %>% group_by(Year) %>%  tally() %>% 
  mutate(Site = "All Sites",
         Year = factor(Year))
...
raw_fig <- bind_rows(trapnet_year,
                     trap_year, site_year) %>% 
  rename(rich = n)
...
B <- chao_fig %>% 
  ggplot(aes(x = Year, y = rich,
             color = factor(Site, level = level_order),
             group = Site)) +
  geom_point(size = 2.5, shape = 15) +
  geom_line(linewidth = 1.1, linetype = "dashed") +
  geom_point(data = raw_fig,
             aes(x = Year, y = rich,
                 color = factor(Site, level = level_order)),
             alpha = 0.3) +
  geom_line(data = raw_fig,
            aes(x = Year, y = rich, group = Site,
                color = factor(Site, level = level_order)),
            linewidth = 0.7,
            alpha = 0.3) +
  scale_color_manual(values = c("#000", "maroon4",
                                "#E69F00", "#56B4E9", "#009E73"),
                     labels = c("All Sites\n(N & T)",
                                "All Sites",
                                "POS", "SCL", "BPF")) +
  labs(x = "", y = "Richness", color = "") +
  theme_cowplot(font_size = 18) +
  expand_limits(y = 175) +
  theme(
    legend.position = c(.15, .9), # Moves legends inside the plot
...
sac_results_4 %>% 
  ...
  scale_color_manual(values = c("#000", "maroon4",
                                "#E69F00", "#56B4E9", "#009E73")) +
  scale_fill_manual(values = c("grey", "maroon4",
                               "#E69F00", "#56B4E9", "#009E73")) +
  theme_cowplot(font_size = 18) +
  labs(x = "No. sampling sites",
       y = "Richness",
...
A <- sac_results_4 %>% 
  ...
  labs(x = "No. sampling sites",
       ...
       fill = "") +
  theme(
    legend.position = c(.45, .4), # Moves legends inside the plot
...
```{r sac_and_chao, echo = F, fig.width = 11}
