...
# Chao indices
```{r chao_indices, echo = F}

# Sites by season
chao_site_season <- dat1 %>% 
  unite("SiteToY", c(Site, Year), sep = "_") %>% 
  group_by(SiteToY, Short.label.name) %>% 
  tally(name = "count") %>% 
  pivot_wider(names_from = SiteToY,
              values_from = count,
              values_fill = list(count = 0)) %>% 
...
chao_site_season <- data.frame(
  ...
  SiteToY = colnames(chao_site_season)) %>% 
  separate_wider_delim(SiteToY, delim = "_",
                       names = c("Site", "ToY"))
